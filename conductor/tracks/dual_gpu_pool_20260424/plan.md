...
- [ ] Implement a worker queue or threading mechanism to track which GPU is busy.
- [ ] **Stream Router:** Route incoming stream requests to the first free GPU.
- [ ] **Batch Splitter:** When a request with `n > 1` arrives, split it. For example, assign `n=1` task to GPU 0 and another `n=1` task to GPU 1 in parallel until all `n` candidates are generated.
- [ ] Ensure strict locking per-GPU so a single GPU is never tasked with concurrent generations.
